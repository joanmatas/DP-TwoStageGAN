...
parser.add_argument(
    "--encoder_lr", type=float, default=1e-4, help="learning rate, default=1e-4"
)
parser.add_argument(
    "--decoder_lr", type=float, default=4e-4, help="learning rate, default=1e-4"
)
parser.add_argument(
    "--D_lr", type=float, default=4e-4, help="learning rate, default=1e-4"
)
parser.add_argument(
    "--grad_clip",
    type=float,
...
parser.add_argument(
    "--save_path", type=str, default="../output/2ndGAN/", help="save path"
)
parser.add_argument("--vis_dir", type=str, default="../output/2ndGAN/vis/", help="vis path")  #
...
epochs_since_G_improvement = (
    0  # keeps track of number of epochs since there's been an improvement
)
...
g_best_loss = 1.0  # best loss score right now
d_best_loss = 1.0
...
def train(
    ...
    for i, data in enumerate(train_loader):
        ...
        esc = data["esc"].to(device)  # (b,4) one-hot indicate four direction   nota: segons dataset.py sembla que esc te dimensio (b,2) i son coordenades, no una codificacio en one-hot
        length = data[
            "len"
        ]  # (b) it seem to be a 1D CPU int64 tensor when use pack_padded_sequence below
        ...
        pred, pred_inv, pred_assemble, sort_ind = decoder(
            imgs, enter, esc, seq[:, :-1, :], seq_inv[:, :-1, :], length - 1
        )
        # pred (b,max_len,2)
        ...
        if i % 4 == 0:  # i % n_critic:
            ...
            errG = nn.BCELoss()(
                logits_gen.view(-1).float(), label.float()
            )  # +nn.MSELoss(reduction='mean')(pred_assemble, seq)*0.5
            ...
            # AttributeError: module 'PIL.Image' has no attribute 'ANTIALIAS' (arreglar en algun moment):
            # writer.add_images(
            #     "pred_grid",
            #     np.reshape(pred_grid, (-1, 32, 32, 1)),
            #     time_step,
            #     dataformats="NHWC",
            # )
        ...
        errD = nn.BCELoss()(logits_real.view(-1).float(), label_real.float()) + nn.BCELoss()(
            logits_gen_d.view(-1).float(), label_fake.float()
        )
        ...
        # writer.add_images(
        #     "real_grid",
        #     np.reshape(real_grid, (-1, 32, 32, 1)),
        #     time_step,
        #     dataformats="NHWC",
        # )
        # writer.add_images(
        ...
        if i % print_freq == 0:
            ...
            print('Prediction:', pred_assemble[0])
...
def validate(val_loader, encoder, decoder, D, criterion, lambd, convsize, std, device):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            pred, pred_inv, predictions_assemble, sort_ind = decoder(
                imgs_encode, enter, esc, seq[:, :-1, :], seq_inv[:, :-1, :], length - 1
            )
            ...
            fake_loss = -(1 - D(predictions_assemble, imgs_encode)[0]) * torch.log(
                1 - D(predictions_assemble, imgs_encode)[0]
            )
            real_loss = -D(seq, imgs_encode)[0] * torch.log(D(seq, imgs_encode)[0])
            ...
            if i % print_freq == 0:
                ...
                print('Preddiction:', predictions_assemble)
    # 'waysloss {waysloss.val:.4f} (Average:{waysloss.avg:.4f})\n'
...
def main():
    global epochs_since_G_improvement, epochs_since_D_improvement, checkpoint, start_epoch, fine_tune_encoder, best_loss, save_path, vis_dir, decoder_dim, lambd, convsize, std, g_best_loss, d_best_loss
    ...
    train_loader = Data.DataLoader(
        dataset.train_set(), batch_size=batch_size, shuffle=False, drop_last=True
    )
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        G_loss, D_loss, imgs, pred, pred_vis, enter, esc, length = validate(
