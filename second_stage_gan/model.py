...
class Decoder(nn.Module):
    ...
    def __init__(self, decoder_dim, encoder_size=4096, encoder_dim=1, attention_dim=16, dropout=0.5, gan = False):
        ...
        # translation: There's a problem here, Does location information need to be done first? embedding? 如果使用 attention and image input, It should be necessary
        # or: There is a question here. Does the location information need to be embedding first? If you use attention and image input, you should need
        self.position_embedding = nn.Linear(2,encoder_size) # 将坐标的信息转化为和图片对齐   translation: Convert coordinate information into alignment with the picture
    ...
    def forward(self, encoder_out, enter, esc, sequence, sequence_inv, seq_len, teach_rate=0.5):
        """
        Forward propagation.
        ...
        for t in range(seq_shape):
            ...
            weight = F.softmax(self.attention(h_a[:batch_size_t]), dim=1) # weight for each input pixels
            weight_inv = F.softmax(self.attention(h_b[:batch_size_t]), dim=1) # (batch_size_t,n_pixels)
        ...
        pred_max = torch.max(predictions_assemble)
        pred_min = torch.min(predictions_assemble)

        return predictions, predictions_inv, torch.div(predictions_assemble, max(pred_max, - pred_min)), sort_ind,
#alphas, alphas_inv
...
class Discriminator(nn.Module):
    ...
    def get_grid(self,p):
        d = 2/self.N
        x = int(torch.div((p[0] + 1), d, rounding_mode='trunc'))
        y = int(torch.div((p[1] + 1), d, rounding_mode='trunc'))
        x = 31 if x >= 32 else x
        y = 31 if y >= 32 else y
        x = 0 if x < 0 else x
        y = 0 if y < 0 else y
