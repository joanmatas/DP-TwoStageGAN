...
parser.add_argument("--batchSize", type=int, default=256, help="inputs batch size")
parser.add_argument(
    "--imageSize",
    type=int,
    default=32,
...
parser.add_argument(
    "--outf", default="../output/DP_WGANGP", help="folder to output images and model checkpoints"
)
...
target_epsilon = 5.0
target_delta = 1e-9     # maxim 1 / n_trajectories
...
def draw_curve(current_epoch):
    ...
    fig.savefig('DP_epsilon' + str(target_epsilon) + '.jpg')
...
def train(dataloader, optimizerD, optimizerG, netD, netG):
    writer = SummaryWriter(log_dir=os.path.join(opt.outf, "log"))
    for epoch in range(opt.start_iter, opt.niter):
        for i, (real_imgs) in enumerate(dataloader):
            ...
            if epoch % 1 == 0:
                ...
                vutils.save_image(
                    (netG(FIXED_NOISE).detach() / 2 + 0.5) * 795,
...
def main():
    ...
    netD, optimizerD, dataloader = privacy_engine.make_private_with_epsilon(
        ...
        epochs=(opt.niter-opt.start_iter),
    ...
    epsilon, alpha = privacy_engine.get_privacy_spent(delta=target_delta)
